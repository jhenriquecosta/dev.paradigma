...
{
    public partial class WorksAgGrid : IDisposable
    {
...
#pragma warning disable CS0649 // Field 'field' is never assigned to, and will always have its default value 'value'
        ...
        private List<AgGridColumnDefinition> _columnDefinitions = new List<AgGridColumnDefinition>();
...
#pragma warning restore CS0649
        ...
        [Parameter] public AgGridOptions Options { get; set; }
        [Parameter] public AgGridEvents Events { get; set; }
        [Parameter] public AgGridCallbacks Callbacks { get; set; }
        [Parameter] public string ConfigureScript { get; set; }

        [Inject] private IJSRuntime JS { get; set; }

        public AgGridApi Api { get; private set; }
        public AgGridColumnApi ColumnApi { get; private set; }
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (!_isRendered)
            {
                _isRendered = true;
                await CreateGrid();
                Api = new AgGridApi(JS, _id);
                ColumnApi = new AgGridColumnApi(JS, _id);
            }
        }
        ...
        private async Task CreateGrid()
        {
            ...
            await JS.InvokeVoidAsync("works_ag_grid.createGrid", _gridDiv,
        ...
        public async void Dispose()
        {
            try
            {
                await JS.InvokeVoidAsync("works_ag_grid.destroyGrid", _gridDiv, _id);
            }
            catch (Exception ex)
            {
